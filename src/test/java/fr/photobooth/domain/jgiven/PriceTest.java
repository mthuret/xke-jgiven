...
@Price
public class PriceTest extends ScenarioTest<GivenAPicture<?>, WhenPaymentIsMade<?>, ThenPhotoBoothRejectsPictures<?>> {

    @ScenarioStage
    private WhenThePictureIsMade<?> $;
    ...
    @As("if the amount of money given to the photo booth is not enough comparing to the picture price, then the picture can't be taken")
    @Test
    public void if_the_amount_of_money_given_to_the_photo_booth_is_not_enough_comparing_the_picture_price_then_the_picture_cant_be_taken() throws Exception {
        ...
        then().the_photo_booth_rejects_it_and_displays_$("not enough money provided : 0.0");
    }
}
